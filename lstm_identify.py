...
def init_data():
    ...
    return np.array(X), np.array(y), max_features, max_len

def main():
    early_stopping_cb = EarlyStoppingCallback(0.99)
    amsgrad = AMSGrad(learning_rate=0.01, beta1=0.9, beta2=0.99, epsilon=1e-8)
    ...
    net = tflearn.lstm(net, 128, bias=True,trainable=True, dropout=0.5)
    ...
    try:
        model.fit(X_train, y_train, validation_set=(X_test, y_test), show_metric=True, n_epoch=100, batch_size=16, callbacks=early_stopping_cb)
